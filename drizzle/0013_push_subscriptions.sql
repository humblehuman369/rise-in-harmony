...
-- Unique index on endpoint so we don't store duplicate subscriptions
CREATE UNIQUE INDEX IF NOT EXISTS `push_subscriptions_endpoint_idx` ON `push_subscriptions` ((LEFT(`endpoint`, 512)));
