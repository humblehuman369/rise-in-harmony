import { COOKIE_NAME, SESSION_ACCESS_MS, decodeOAuthState } from "@shared/const";
...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    // Note: the CSRF nonce is stored in the client's sessionStorage (not a cookie)
    // so it cannot be validated server-side. The nonce is included in the state
    // for future use with a server-side session store if needed.
    // The Manus OAuth server itself validates the code/redirectUri binding,
    // which provides the primary CSRF protection for this flow.
